...
class RosbridgeProtocolCBOR(RosbridgeProtocol):
    ...
    def __init__(self, client_id, parameters):
        ...
        self.log("warning", "Starting RosbridgeProtocolCBOR (extended to receive CBOR)")

    # We override incoming losing fragmentation but adding cbor support
    def incoming(self, message_string=""):
        # self.log("debug", "Incoming message_string: %s" % message_string)
        if self.bson_only_mode:
            ...
            # self.log("debug", "Decoded message_string: %s" % msg)
